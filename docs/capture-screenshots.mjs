...
assert.equal(manifest.version, '0.4.5', 'These README fixtures document v0.4.5')
...
function shell(theme, mode) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><style>
    ...
    <main id="demo-copy"><div class="demo-eyebrow">${modal ? '独立面板 · 原生 DIALOG' : historyMode ? 'BETTER SIDEBAR · 执行记录' : 'BETTER SIDEBAR · 定时任务'}</div>
      ...
      <section class="demo-card"><h2>截图范围说明</h2><p>右侧为 v0.4.5 已提交构建中的真实组件。<br>外围外壳${modal ? '' : '、侧栏服务'}与任务 API 均为隔离夹具。<br>未连接 DSH，也未创建或运行任何真实任务。</p></section>
    </main>
  ...
  for (const [filename, theme, mode] of [
    ...
    try {
      ...
      if (mode === 'standalone') {
        ...
        assert.equal(await panel.locator('.dsh-cron-owner').isVisible(), true, 'Standalone retains explicit owner')
      } else {
        assert.equal(await page.locator('dialog').count(), 0)
        assert.equal(await page.locator('#demo-tab-title').innerText(), '定时任务')
        assert.equal(await panel.locator('.dsh-cron-drawerTitle').count(), 0, 'No repeated embedded title')
        assert.equal(await panel.locator('.dsh-cron-owner').isVisible(), false, 'Owner not permanently displayed')
        assert.equal(await page.locator('.dsh-cron-triggerCompact').count(), 1, 'Header uses compact clock entry')
      }
