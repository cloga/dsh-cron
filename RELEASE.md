...
- Exact Core checks cover `a66e4702047846cdaa10c66c9d3df3951f5ea70d` (0.1.2-rc.1), `d347e703908d0406b7a7ef80e3a0e594d86b2215` (0.1.3-alpha.1), `5dda764ed3aa172535a7967b06ff95d9cbfe536a` (0.1.5-alpha.1), `b2e3b2a0125854567a4a5fcba75782e42fe84901` (0.1.5-alpha.2), and `fb2c4b9e698e30edb738bca4cf0618587db7d203` (0.1.5-rc.2). CI runs all five on Windows/Linux and Node 22.19/24; the browser job runs separately on Ubuntu/Node 24. Use `DSH_CORE_PATH` for a clean checkout, or additionally set `DSH_CORE_REF` to read that immutable commit from an existing Git repository without altering its checkout. A ref must resolve to an allowlisted exact SHA; checkout mode still rejects tracked modifications.
- Report evidence precisely: package peer policy is not source verification; exact declarations and source-backed 0.1.5 JSONL handle-to-Cron tests use fake storage/agents, not full Core startup, JSONL migration/IO, a real model, or live GUI. Explicitly report source-check skips. Version 0.4.8 is a backward-compatible compatibility patch for issue #32, adding only official 0.1.5-rc.2 while retaining the 0.4.7 dual-read normalization (#30, alpha.1 #29). The rc.2 persistence/JSONL handle source is unchanged from alpha.2; execute its own exact Git class and verify public Slot scopes anyway. This is not a general 0.1.5 compatibility promise. An uncommitted candidate can receive only a worktree assessment; the important-release gate reads committed HEAD and must be rerun after the complete candidate is committed.
...
5. The release job re-verifies all five exact Core source contracts (including source-backed 0.1.5 handle regressions), browser regression and committed bundle, packs the artifact and generates/validates `SHA256SUMS`.
